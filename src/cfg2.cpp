#include "precompiled.h"
#include "cfg2.h"
#include "CinderImGui.h"


void cfg2::init()
{
	ImGui::Initialize();
	ImGuiIO& io = ImGui::GetIO();
	io.Fonts->AddFontFromFileTTF("c:\\Windows\\Fonts\\segoeui.ttf", 18.0f);
}

void cfg2::begin()
{
	ImGui::Begin("Parameters");
}

void cfg2::end()
{
	ImGui::End();
}

template<class T> T& getOpt_Base(string const& name, T defaultValue, bool* isFirstTime) {
	static map<string, T> m;
	*isFirstTime = false;
	if (!m.count(name)) {
		m[name] = defaultValue;
		*isFirstTime = true;
	}
	return m[name];
}

int cfg2::getInt(string const& name, int min, int max, int defaultValue, ImGuiSliderFlags flags) {
	bool isFirstTime;
	auto& ref = getOpt_Base<int>(name, defaultValue, &isFirstTime);
	if(isFirstTime)
		ImGui::DragInt(name.c_str(), &ref, 1.0f, min, max, "%d", flags);
	return ref;
}

float cfg2::getFloat(string const& name, float speed, float min, float max, float defaultValue, ImGuiSliderFlags flags) {
	bool isFirstTime;
	auto& ref = getOpt_Base<float>(name, defaultValue, &isFirstTime);
	if (isFirstTime)
		ImGui::DragFloat(name.c_str(), &ref, speed, min, max, "%.3f", flags);
	return ref;
}
bool cfg2::getBool(string const& name, bool defaultValue) {
	bool isFirstTime;
	auto& ref = getOpt_Base<bool>(name, defaultValue, &isFirstTime);
	if (isFirstTime)
		ImGui::Checkbox(name.c_str(), &ref);
	return ref;
}
