...
namespace ThisSketch {
	...
	std::vector<Img> buildGaussianPyramidGpu(Img src, float scalePerLevel = 0.5f, float downscaleSigma = -1.0f);
	...
	Array2D<float> resizeGaussianCpuSimple(Array2D<float> src, ivec2 dstSize, float sigma = -1.0f);
